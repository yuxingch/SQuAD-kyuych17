...
from modules import RNNEncoder, SimpleSoftmaxLayer, BasicAttn, GatedAttn, SelfAttn, PointerNetwork
...
class QAModel(object):
    ...
    def __init__(self, FLAGS, id2word, word2id, emb_matrix):
        ...
        with tf.variable_scope("QAModel", initializer=tf.contrib.layers.variance_scaling_initializer(factor=1.0, uniform=True)):
            ...
            # self.add_embedding_layer(emb_matrix, use_char_level=False)
            self.build_graph()
            self.add_loss()
    ...
    def add_placeholders(self):
        ...
        # Add a placeholder to feed in the character IDs for e.g. the context
        self.character_ids = tf.placeholder(tf.int32, shape=[None, self.FLAGS.context_len, self.FLAGS.word_len])
    ...
    def build_graph(self):
        ...
        context_hiddens = self_attn_layer.build_graph(context_hiddens, self.context_mask)

        # R-NET: pointer network output layer
        ptr_net = PointerNetwork()
        #logits, dist = ptr_net.build_graph(question_hiddens, context_hiddens, self.qn_mask, self.context_mask)
        #self.logits_start = tf.slice(logits, [0, 0, 0], [logits.shape.as_list()[0], 1, logits.shape.as_list()[2]])
