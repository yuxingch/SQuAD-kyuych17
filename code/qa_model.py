...
def charCNN(input_char_ids, filter_size=100, window_size=5):
    ...
    # max pooling
    # max_pooling = tf.layers.max_pooling2d(inputs=conv2, pool_size=2, strides=2, padding='same') 
...
class QAModel(object):
    ...
    def __init__(self, FLAGS, id2word, word2id, emb_matrix, id2char, char2id):
        ...
        with tf.variable_scope("QAModel", initializer=tf.contrib.layers.variance_scaling_initializer(factor=1.0, uniform=True)):
            ...
            # need to modify this? Currently it add the word-level word embeddings, need to find the char-level embeddings and concatenate
    ...
    def add_embedding_layer(self, emb_matrix):
        ...
        TODO:
            add the character-level embeddings
        """
        with vs.variable_scope("embeddings"):
            ...
            with vs.variable_scope("character_level_cnn", reuse=tf.AUTO_REUSE) as scope:
                ...
                # print reshaped_context_char_ids.get_shape()              
                reshaped_context_char_embs = embedding_ops.embedding_lookup(char_emb_matrix, reshaped_context_char_ids)
                # print reshaped_context_char_embs.get_shape()
                ...
                reshaped_qn_char_ids = tf.reshape(self.qn_char_ids, [-1, self.FLAGS.word_len*self.FLAGS.question_len])               
                reshaped_qn_char_embs = embedding_ops.embedding_lookup(char_emb_matrix, reshaped_qn_char_ids)
    ...
    def train(self, session, train_context_path, train_qn_path, train_ans_path, dev_qn_path, dev_context_path, dev_ans_path):
        ...
        while self.FLAGS.num_epochs == 0 or epoch < self.FLAGS.num_epochs:
            ...
            # Loop over batches
            for batch in get_batch_generator(self.word2id, self.char2id, train_context_path, train_qn_path, train_ans_path, self.FLAGS.batch_size, context_len=self.FLAGS.context_len, word_len=self.FLAGS.word_len, question_len=self.FLAGS.question_len, discard_long=True):
